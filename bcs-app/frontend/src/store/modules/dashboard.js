...
export default {
    ...
    actions: {
        ...
        getListByCategory (context, params, config = {}) {
            const { projectId, clusterId, category } = params

            return http.get(
                `${DEVOPS_BCS_API_URL}/api/dashboard/projects/${projectId}/clusters/${clusterId}/workloads/${category}/`,
                {},
                config
            )
        },

        subscribeList (context, params, config = {}) {
            const { projectId, clusterId, data } = params

            return http.get(
                `${DEVOPS_BCS_API_URL}/api/dashboard/projects/${projectId}/clusters/${clusterId}/subscribe/?${json2Query(data)}`,
                {},
                config
            )
        },

        getNamespaceList (context, params, config = {}) {
            const { projectId, clusterId } = params

            return http.get(
                `${DEVOPS_BCS_API_URL}/api/dashboard/projects/${projectId}/clusters/${clusterId}/namespaces/`,
                {},
                config
            )
        },
