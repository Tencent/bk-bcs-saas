                    ...
                    <div class="biz-tab-content" v-bkloading="{ isLoading: isTabChanging }">
                        <template v-if="!HPAs.length">
                            <p class="biz-template-tip f12 mb10">
                                {{$t('HPA是Mesos自动扩缩容服务，利用HPA能够根据监测到 cpu、memory 的利用率')}}，<a class="bk-text-button" :href="PROJECT_CONFIG.doc.mesosHpa" target="_blank">{{$t('详情查看文档')}}</a>
                            </p>
                                ...
                                <div class="bk-form biz-configuration-form">
                                    ...
                                    <div class="bk-form-item is-required">
                                        ...
                                        <div class="bk-form-content" style="margin-left: 130px;">
                                            ...
                                            <span class="biz-tip ml10" v-if="!isDataLoading && !applicationList.length">{{$t('请先配置Application，再进行关联')}}</span>
                                        </div>
                                    </div>
                                        ...
                                        <div class="bk-form-content" style="margin-left: 130px;">
                                            <div class="bk-form-input-group is-addon-left mr10">
                                                ...
                                                <bk-input
                                                    ...
                                                    style="width: 105px;"
                                                    :min="1"
                                                    :value.sync="curHPA.config.spec.minInstance"
                                                    :list="varList">
                                                </bk-input>
                                            </div>
                                            ...
                                            <div class="bk-form-input-group is-addon-left" style="display: inline-block;">
                                                ...
                                                <bk-input
                                                    ...
                                                    :placeholder="$t('请输入')"
                                                    style="width: 103px;"
                                                    :min="curHPA.config.spec.minInstance"
                                                    :value.sync="curHPA.config.spec.maxInstance"
                                                    :list="varList">
                                                </bk-input>
                                            </div>
                                        </div>
                                                        ...
                                                        <td>
                                                            <div class="bk-form-input-group">
                                                                <bk-input
                                                                    ...
                                                                    :min="0"
                                                                    :placeholder="$t('请输入')"
                                                                    :value.sync="metric.target.averageUtilization">
                                                                </bk-input>
...
<script>
    import hpaParams from '@open/json/hpa.json'
    ...
    export default {
        ...
        methods: {
            ...
            updateLocalData (data) {
                ...
                this.$store.commit('mesosTemplate/updateHPAs', this.HPAs)
                setTimeout(() => {
                ...
                this.$bkInfo({
                    ...
                    async confirmFn () {
                        if (HPAId.indexOf && HPAId.indexOf('local_') > -1) {
                            ...
                            try {
                                const res = await self.$store.dispatch('mesosTemplate/removeHPA', { HPAId, version, projectId })
                                const data = res.data
                                self.removeLocalHPA(hpa, index)
            ...
            async getHPAMetric () {
                try {
                    const res = await this.$store.dispatch('mesosTemplate/getHPAMetric', this.projectId)
            ...
            async initApplications (version) {
                ...
                try {
                    const res = await this.$store.dispatch('mesosTemplate/getDeploymentsByVersion', { projectId, version })
            ...
            showJsonPanel () {
                this.toJsonDialogConf.title = this.curHPA.config.metadata.name + '.json'
