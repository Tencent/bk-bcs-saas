        ...
        <template v-if="curCluster.type === 'mesos' && curCluster.func_wlist && curCluster.func_wlist.indexOf('MesosResource') > -1">
            <div class="biz-progress-box" style="margin-bottom: 15px;">
                ...
                <div style="margin-bottom: 2px;">
                    {{curCluster.activeip}} / {{curCluster.availableip + curCluster.activeip}}（{{ $t('剩余') }}{{curCluster.availableip}}）
                </div>
            ...
            <div class="biz-progress-box" style="margin-bottom: 15px;">
                ...
                <div style="margin-bottom: 2px;">
                    {{(curCluster.cpu_usage.total - curCluster.cpu_usage.remain).toFixed(2)}} / {{curCluster.cpu_usage.total}}（{{ $t('剩余') }}{{parseFloat(curCluster.cpu_usage.remain).toFixed(2)}}）
                </div>
                <div class="progress" :class="loading ? 'loading' : ''">
            ...
            <div class="biz-progress-box" style="margin-bottom: 15px;">
                ...
                <div style="margin-bottom: 2px;">
                    {{(curCluster.mem_usage.total / 1024 - curCluster.mem_usage.remain / 1024).toFixed(2)}} / {{(curCluster.mem_usage.total / 1024).toFixed(2)}}（{{ $t('剩余') }}{{(curCluster.mem_usage.remain / 1024).toFixed(2)}}）
                </div>
                <div class="progress" :class="loading ? 'loading' : ''">
