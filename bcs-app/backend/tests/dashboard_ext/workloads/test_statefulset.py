...
class TestStatefulSet:
    ...
    def test_list(self, api_client, project_id, cluster_id, dashboard_api_common_patch):
        ...
        response = api_client.get(
            f'/api/dashboard/projects/{project_id}/clusters/{cluster_id}/workloads/statefulsets/'
        )
        assert response.json()['code'] == 0

    def test_retrieve(self, api_client, project_id, cluster_id, dashboard_api_common_patch):
        """ 测试获取单个资源接口 """
        namespace, statefulset_name = 'default', 'test_statefulset_name'
        response = api_client.get(
            f'/api/dashboard/projects/{project_id}/clusters/{cluster_id}/'
            + f'namespaces/{namespace}/workloads/statefulsets/{statefulset_name}/'
        )
        assert response.json()['code'] == 0
