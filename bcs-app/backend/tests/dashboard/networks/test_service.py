...
class TestService:
    ...
    manifest = load_demo_manifest(K8sResourceKind.Service.value)
    name = getitems(manifest, 'metadata.name')
    batch_url = f'{DAU_PREFIX}/networks/services/'
    detail_url = f'{DAU_PREFIX}/namespaces/{DEFAULT_NAMESPACE}/networks/services/{name}/'
