...
class TestPersistentVolumeClaim:
    ...
    manifest = load_demo_manifest(K8sResourceKind.PersistentVolumeClaim.value)
    name = getitems(manifest, 'metadata.name')
    batch_url = f'{DAU_PREFIX}/storages/persistent_volume_claims/'
    detail_url = f'{DAU_PREFIX}/namespaces/{DEFAULT_NAMESPACE}/storages/persistent_volume_claims/{name}/'
