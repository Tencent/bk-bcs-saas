...
def create_cluster(access_token: str, project_id: str, cc_app_id: int, data: Dict) -> Dict:
    ...
    try:
        ...
        cluster_config = bcs_api.ClusterConfig(
            creator=data["creator"], bcs_cluster_config=bcs_cluster_config, cloud_cluster_config=cloud_cluster_config
        )
...
def delete_cluster(access_token: str, project_id: str, cluster_id: str):
    """删除集群"""
    bcs_cc_client = paas_cc.PaaSCCClient(comp_base.ComponentAuth(access_token))
...
def add_nodes(access_token: str, project_id: str, cluster_id: str, data: Dict):
    ...
    try:
        client = bcs_api.BcsApiClient(comp_base.ComponentAuth(access_token))
        node_config = bcs_api.NodeConfig(nodes=[node["inner_ip"] for node in data["objects"]], onlyCreateInfo=True)
