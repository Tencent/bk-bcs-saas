...
from backend.components.cc import get_biz_internal_module
...
class CCHostListViewSetOLD(NodeBase, NodeHandler, viewsets.ViewSet):
    renderer_classes = (BKAPIRenderer, BrowsableAPIRenderer)
    ...
    def get_cc_host_mappings(self, host_list):
        data = {info['InnerIP']: convert_mappings(constants.CCHostKeyMappings, info) for info in host_list}
        return data
    ...
    def post(self, request, project_id):
        ...
        if not host_list:
            return Response({'results': [], 'cc_application_name': self.cc_application_name})
...
class CCHostListViewSet(SystemViewSet):
    """ 获取 CMDB 可用主机列表 """
    ...
    def create(self, request, project_id):
        ...
        cmdb_client = cmdb.CMDBClient(request)
        host_list = self._fetch_cc_app_hosts(cmdb_client, idle_only=params['idle_only'])
        cc_app_name = cmdb_client.get_cc_application_name()
        ...
        # 字段映射，前端展示需要
        ret['results'] = self._field_mappings(ret['results'])
    ...
    def _fetch_cc_app_hosts(self, cmdb_client, idle_only: bool = False) -> List[Dict]:
        """
        拉取 业务 下机器列表（全量/空闲机模块）

        :param cmdb_client: CMDB Client
        :param idle_only: 若为 true，则只拉取空闲机模块下机器
        :return: CMDB 业务下机器列表
        """
        # 查询当前业务对应的空闲机模块
        bk_module_ids = None
        if idle_only:
            idle_module = cmdb_client.get_idle_module_info()
            bk_module_ids = [idle_module.get('bk_module_id')]

        return cmdb_client.get_cc_hosts(bk_module_ids)
    ...
    def _field_mappings(self, host_list: List) -> List:
        """ 对结果进行字段转换，用于前端展示 TODO 该逻辑能否废弃，让前端直接使用原始字段？ """
        return [convert_mappings(constants.CCHostKeyMappings, info) for info in host_list]

    def _update_gse_agent_status(self, host_list: List) -> List:
        """ 更新 GSE Agent 状态信息 """
        cc_host_map = {host['InnerIP']: host for host in host_list}
        gse_host_status_map = {
            info['ip']: info for info in gse.GSEClient.get_agent_status(self.request.user.username, host_list)
        }
        for ips in cc_host_map:
