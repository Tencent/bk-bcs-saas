...
class MesosClient(BCSClientBase):
    ...
    def create_application(self, namespace, data):
        """创建application
        """
        url = "{host}/mesos/namespaces/{ns}/applications".format(
    ...
    def rollback_mesos_app_instance(self, namespace, data):
        """回滚application
        """
        url = "{host}/mesos/namespaces/{ns}/applications/rollback".format(
            host=self.scheduler_host, ns=namespace
        )
    ...
    def get_mesos_app_taskgroup(self, taskgroup_name=None, namespace=None,
                                app_name=None, field=None):
        """获取taskgroup
        """
        url = "{host}/query/mesos/dynamic/clusters/{cluster_id}/taskgroup".format(
            host=self.storage_host, cluster_id=self.cluster_id
    ...
    def create_configmap(self, namespace, data):
        url = "{host}/mesos/namespaces/{ns}/configmaps".format(
            host=self.scheduler_host, ns=namespace
        )
    ...
    def disable_agent(self, ip):
        """停用agent，不允许再被调度
        """
        url = "{host}/mesos/agentsettings/disable".format(
    ...
    def enable_agent(self, ip):
        """启用Agent
        """
        url = "{host}/mesos/agentsettings/enable".format(
            host=self.scheduler_host)
        params = {"ips": ip}
        resp = http_post(url, params=params, headers=self.headers)
        return resp
    ...
    def delete_secret(self, namespace, name):
        """删除secret
        """
        url = "{host}/mesos/namespaces/{ns}/secrets/{name}".format(
            host=self.scheduler_host, ns=namespace, name=name
        )
    ...
    def create_service(self, namespace, data):
        url = "{host}/mesos/namespaces/{ns}/services".format(
            host=self.scheduler_host, ns=namespace
        )
        resp = http_post(url, json=data, headers=self.headers)
        return resp
    ...
    def create_deployment(self, namespace, data):
        url = "{host}/mesos/namespaces/{ns}/deployments".format(
            host=self.scheduler_host, ns=namespace
        )
    ...
    def set_metrics(self, data, cluster_type='mesos'):
        """添加监控
        """
        url = '{host}/clustertype/{cluster_type}/metrics'.format(
    ...
    def update_agent_attrs(self, attrs):
        """批量修改/增加节点属性
        """
        url = "{host}/mesos/agentsettings".format(
            host=self.scheduler_host
        )
