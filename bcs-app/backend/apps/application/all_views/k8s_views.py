...
class GetNamespace(object):
    ...
    def get_namespaces(self, request, project_id):
        resp = paas_cc.get_namespace_list(
            ...
            project_id, desire_all_data=True
        )
    ...
    def get_ns_app_info(self, ns_id_list, category, inst_name, project_id, request):
        ...
        all_info = InstanceConfig.objects.filter(
            namespace__in=ns_id_list, is_deleted=False, category=category
    ...
    def get_ns_inst_error_count(
            self, func, request, project_id, category,
            kind, ns_name_list, inst_name, cluster_id_list, ns_name_inst):
        """获取实例错误数据
        """
        error_data = {}
        all_data = {}
    ...
    def get(
            self, request, ns_id_list, category, ns_map, project_id,
            kind, func, inst_name, ns_name_list, cluster_id_list):
        ...
        ns_inst_error_count, all_ns_inst_count = self.get_ns_inst_error_count(
            func, request, project_id, category,
            kind, ns_name_list, inst_name, cluster_id_list, ns_name_inst
        )
...
class GetInstances(object):
    ...
    def get_insts(self, ns_id, category, inst_name):
        ...
        all_inst_list = InstanceConfig.objects.filter(
            namespace=ns_id, category=category, is_deleted=False
        ).exclude(ins_state=InsState.NO_INS.value).order_by("-updated")
        if inst_name:
    ...
    def get(self, request, project_id, ns_id, category, inst_name,
            app_status, cluster_env_map, cluster_id, ns_name, ns_name_id):
        """获取命名空间下的实例
        """
        ...
        # 进行匹配
        inst_status_info = self.get_k8s_category_info(
            request, project_id, category, inst_name, cluster_id, ns_name
        )
