...
# 模板集删除时, 需要一起删除的资源
SHOULD_DELETE_CATEGORY = ["service", "secret", "configmap", "K8sSecret", "K8sConfigMap", "K8sService", "K8sIngress", "K8sHPA"]
...
class BaseAPI(views.APIView):
    ...
    def get_project_kind(self, request, project_id):
        ...
        """
        project_info = paas_cc.get_project(
    ...
    def get_k8s_pod_info(
        ...
        resp = client.get_pod(
            extra=extra_encode, field=",".join(field_list), params=params
        )
        if resp.get("code") != 0:
            raise error_codes.APIError.f(
                resp.get("message") or "查询出现异常",
    ...
    def get_pod_or_taskgroup(self, request, project_id, cluster_id, field=None, app_name=None,
                             taskgroup_name=None, ns_name=None, kind=2, category=None):
        """获取taskgroup或者pod
        """
        if kind == 2:
            client = MesosClient(
            ...
            if taskgroup_name:
                ...
                if category in ["deployment", "K8sDeployment"]:
                    rs_name = self.get_k8s_rs_info(
                        request, project_id, cluster_id, ns_name, app_name)
                    if not rs_name:
                        return True, []
            else:
                pod_name = None
    ...
    def create_instance(self, request, project_id, cluster_id, ns, data,
                        category="application", kind=2):
        """创建实例
        """
        if kind == 2:
            client = MesosClient(
    ...
    def delete_instance(self, request, project_id, cluster_id, ns, instance_name,
                        category="application", kind=2, inst_id_list=None, enforce=0):
        """删除instance
        """
        if kind == 2:
            client = MesosClient(
            ...
            if inst_id_list:
                delete_instance_task.delay(
                    request.user.token.access_token, inst_id_list, kind
                )
        else:
            client = K8SClient(
                request.user.token.access_token,
    ...
    def get_mesos_application_deploy_status(
        ...
        if category == "application":
            resp = client.get_mesos_app_instances(
                ...
                field=field or "data.metadata.name,data.metadata.namespace,data.status,data.message",  # noqa
                namespace=namespace,
            )
        else:
    ...
    def get_k8s_application_deploy_status(
            self, request, project_id, cluster_id, instance_name,
            category="application", namespace=None, field=None):
    ...
    def get_application_deploy_info(self, request, project_id, cluster_id, instance_name,
                                    category="application", project_kind=2, namespace=None, field=None):
        """获取详情
        """
        if project_kind == 2:
    ...
    def get_app_deploy_with_post(self, request, project_id, cluster_id, instance_name=None,
                                 category="application", project_kind=2, namespace=None, field=None):
        """获取详情
        """
        if project_kind == 2:
            return self.get_mesos_app_deploy_with_post(
                request, project_id, cluster_id, instance_name=instance_name,
                category=category, namespace=namespace, field=field,
            )
        else:
    ...
    def update_deployment(
            self, request, project_id, cluster_id, ns, data, kind=2, category=None, app_name=None):
        """滚动升级
        """
        if kind == 2:
            client = MesosClient(
    ...
    def rescheduler_taskgroup(self, request, project_id, cluster_id, ns,
                              instance_name, taskgroup_name, kind=2):
        """重启更新
        """
        if kind == 2:
            client = MesosClient(
    ...
    def update_instance_record_status(self, info, oper_type, status="Running", deleted_time=None,
                                      category=None, created=None, inst_state=None, is_deleted=None,
                                      is_bcs_success=None):
        """更新单条记录状态
        """
        ...
        if category in SHOULD_DELETE_CATEGORY:
    ...
    def get_rc_name_by_deployment_base(self, request, project_id, cluster_id, instance_name,
    ...
    def get_mesos_application_deployment_config(
            self, access_token, project_id, cluster_id, name, namespace, category):
