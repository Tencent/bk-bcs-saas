...
class TemplateNamespace(BaseAPI):
    ...
    def get_ns_info(self, request, project_id, ns_id_list):
        ...
        resp = paas_cc.get_namespace_list(
            request.user.token.access_token, project_id, limit=constants.ALL_LIMIT
        )
...
class DeleteTemplateInstance(BaseAPI):
    ...
    def get_instance_info(self, ns_id_list, name, category=None):
        ...
        inst_info = InstanceConfig.objects.filter(
            name__in=name, namespace__in=ns_id_list,
            is_deleted=False
        ).exclude(
    ...
    def delete_all(self, request, data, project_id, muster_id, show_version_name, res_name, muster_name):
        ...
        # 获取要删除的实例信息
        inst_info = self.get_instance_info(
            ns_id_list, tmpl_category_name_map.keys())
    ...
    def delete_handler(self, request, inst_info, project_id, project_kind):
        ...
        for inst_id, info in inst_info.items():
            ...
            self.bcs_single_app_perm_handler(
                request, project_id, info["muster_id"], info["info"].namespace)
            # 针对0/0的情况先查询一次
            if info["info"].category in app_constants.ALL_CATEGORY_LIST:
                if not self.get_category_info(
                    request, project_id, info["cluster_id"], project_kind, info["instance_name"],
                    info["namespace"], info["info"].category
                ):
                    deleted_id_list.append(inst_id)
                    continue
            resp = self.delete_instance(
                request, project_id, info["cluster_id"], info["namespace"], info["instance_name"],
            ...
            if info["info"].category in SKIP_CATEGORY:
                self.update_instance_record_status(
                    info["info"], app_constants.DELETE_INSTANCE, status="Deleting"
                )
            else:
                self.update_instance_record_status(
                    info["info"], app_constants.DELETE_INSTANCE,
                    status="Deleted", category=info["info"].category,
    ...
    def delete(self, request, project_id, muster_id):
        ...
        if data.get("id_list") and show_version_name:
            return self.delete_all(
                request, data, project_id, muster_id, show_version_name, res_name, muster_name
            )
        else:
