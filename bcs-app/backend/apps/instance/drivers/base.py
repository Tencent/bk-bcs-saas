        ...
        for res, specs in config:
            ...
            for spec in specs:
                ...
                if res not in ['application', 'deployment']:
                    self.rollback_stack[ns_id].append(
                        [res, ns, cluster_id, spec['config']])

                # 获取状态信息
                if res == 'metric':
                    queryset = MetricConfig.objects.filter(
                        pk=spec['instance_config_id'])
                else:
                    queryset = InstanceConfig.objects.filter(
                        pk=spec['instance_config_id'])
                # 需要更新的参数
                ...
                try:
                    ...
                    result = error.args[0]
                    InstanceEvent.log(
                        spec['instance_config_id'],
                        res,
                        EventType.REQ_FAILED.value,
                        result,
                        spec['context'])
    ...
    def handler_rollback(self, ns_id):
        ...
        for s in roll_back_list:
            ...
            if handler:
                ...
                logging.warning(
                    "have not rollback handler, %s, %s, will ignore", s, s[1:])
