...
urlpatterns = [
    # 保存用户可见版本
    url(r'^api/configuration/(?P<project_id>\w{32})/template/(?P<template_id>\d+)/show/version/$',
        views.ShowVersionViewSet.as_view({'get': 'list_show_versions', 'post': 'save_with_ventity'})),

    # 根据模板集查询用户可见版本列表/ 创建可见版本
    url(r'^api/configuration/(?P<project_id>\w{32})/show/versions/(?P<template_id>\d+)/$',
