...
urlpatterns = [
    url(r'^api/projects/(?P<project_id>\w{32})/network/clb/names/$',
        views.DescribeCLBNamesViewSet.as_view({'get': 'list'})),
    url(r'^api/projects/(?P<project_id>\w{32})/network/clbs/$',
        views.CLBListCreateViewSet.as_view({'get': 'list', 'post': 'create'})),
    url(r'^api/projects/(?P<project_id>\w{32})/network/clbs/(?P<clb_id>\d+)/$',
