...
class CLBListCreateViewSet(viewsets.ViewSet):
    ...
    def add_status_and_cluster_name(self, request, project_id, data):
        ...
        for cluster_id in cluster_id_list:
            clb_dict.update(clb_utils.get_deployments(
    ...
    def create(self, request, project_id):
        slz = serializers.CreateCLBSLZ(data=request.data, context={"request": request})
...
class MesosCLBOperateViewSet(viewsets.ViewSet):
    ...
    def render_image_url(self, access_token, project_id, record):
        """渲染image url
        """
        if not record.get("is_customized_image_url", False):
            repo_domain = paas_cc.get_jfrog_domain(
                access_token, project_id, record["cluster_id"])
            if not repo_domain:
                repo_domain = settings.DEFAUT_MESOS_LB_JFROG_DOMAIN
            # 兼容处理，去掉右侧的"/"
            record["clb_controller_image_url"] = f"{repo_domain}/{record['clb_controller_image_url'].lstrip('/')}"
            record["clb_nginx_image_url"] = f"{repo_domain}/{record['clb_nginx_image_url'].lstrip('/')}"
    ...
    def post(self, request, project_id, clb_id):
        ...
        self.render_image_url(access_token, project_id, record)
...
class CLBRetrieveOperateViewSet(viewsets.ViewSet):
    ...
    def add_status(self, request, data):
        ...
        cluster_id_clb_name = (data["cluster_id"], data["resource_name"])
        data.update(deployment_status.get(cluster_id_clb_name, {}))
        return data
