...
def get_deployments(access_token, project_id, project_kind, cluster_id, name=None):
    ...
    for info in data:
        ...
        deployment_dict[(cluster_id, metadata.get('name'))] = {
            'clb_status': info_data.get('status'), 'clb_message': info_data.get('message')
        }
...
def request_clb_status(request, project_id, record):
    pod_ip = get_taskgroup_ip(
        request, project_id, record['cluster_id'], record['resource_name'], record['namespace'])
