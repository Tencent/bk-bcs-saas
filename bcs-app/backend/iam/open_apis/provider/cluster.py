...
from backend.container_service.clusters.base import filter_clusters, get_clusters
...
class ClusterProvider(ResourceProvider):
    ...
    def list_instance(self, filter_obj: FancyDict, page_obj: Page, **options) -> ListResult:
        ...
        Parameters
        ----------
        filter_obj: 查询参数字典。 以下为必传
            parent (dict): 父类 即project
                id (int): 父类id 即所属project的id
        page_obj: 分页对象
        options: 可选参数
        ...
        results = [
            {'id': f"{project_id}:{cluster['cluster_id']}", 'display_name': cluster['name']}
            for cluster in cluster_slice
        ]
    ...
    def fetch_instance_info(self, filter_obj: FancyDict, **options) -> ListResult:
        ...
        """
        project_id = filter_obj.parent["id"]
        cluster_ids = filter_obj.ids or []
        cluster_list = filter_clusters(get_system_token(), project_id, cluster_ids)
        results = [
            {'id': f"{project_id}:{cluster['cluster_id']}", 'display_name': cluster['name']}
